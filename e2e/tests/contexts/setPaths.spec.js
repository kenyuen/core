  ...
  describe('when manipulated by current app ', function () {
    ...
    it("should invoke the callback registered with subscribe() method when there's a subscription for the context", (done) => {
      ...
      glue.contexts
        ...
        .then(() => glue.contexts.setPaths(contextName, [{ path: 'prop', value: 'value' }]))
        .catch((err) => done(err));
    });
    ...
    it('should invoke the callback registered with subscribe() method 3 times when a context is updated 3 times', (done) => {
      ...
      glue.contexts
        ...
        .then(() => glue.contexts.setPaths(contextName, [{ path: 'prop1', value: prop1Context }]))
  ...
  describe('when manipulated by another app, ', function () {
    ...
    it("should invoke the callback registered with subscribe() method when there's a subscription for the context", (done) => {
      ...
      glue.contexts
        ...
        .then(() => supportApp.contexts.setPaths(contextName, [{ path: 'prop', value: 'value' }]))
        .catch((err) => done(err));
    });
    ...
    it('should invoke the callback registered with subscribe() method 3 times when a context is updated 3 times', (done) => {
      ...
      glue.contexts
        ...
        .then((unsubscribeFn) => gtf.addWindowHook(unsubscribeFn))
        .then(() => supportApp.contexts.setPaths(contextName, [{ path: 'prop1', value: prop1Context }]))
